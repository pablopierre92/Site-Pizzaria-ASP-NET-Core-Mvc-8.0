...
namespace Pizzaria.Services.Pizza
{
	public class PizzaService : IPizzaInterface
	{
		...
		{
			var codigoUnico = Guid.NewGuid().ToString(); //gera uma cadeia de caracteres na frente do nome da imagem para evitar conflitos
			var nomeCaminhoImagem = foto.FileName.Replace(" ", "").ToLower() + codigoUnico + ".png"; //remove os espaços e acrescenta o codigo unico
			...
			using (var stram = File.Create(caminhoParaSalvarImagens + nomeCaminhoImagem))
			{
				foto.CopyToAsync(stram).Wait();
			}
